...
>(
  ...
  // preloadedState 复制给 enhancer，preloadedState 置为 undefined
  ...
  let currentState = preloadedState as S; // 初始化 state
  let currentListeners: (() => void)[] | null = []; //订阅的回调数组;
  ...
  function subscribe(listener: () => void) {
    ...
    ensureCanMutateNextListeners(); // 浅拷贝,将currentListeners 拷贝到nextListeners
   ...
   */
  // 根据 action.type 执行 reducer 函数，触发 state 更新，然后发布通知即遍历 nextListeners 执行
